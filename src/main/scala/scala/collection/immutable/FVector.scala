...
//
//  def empty[A]: FVector[A] = FVector0
//
//  def from[E](it: collection.IterableOnce[E]): FVector[E] =
//    it match {
//      case v: FVector[E] => v
//      case _ =>
...
//    }
//
//  def newBuilder[A]: ReusableBuilder[A, FVector[A]] = new FVectorBuilder[A]
//
//  /** Create a FVector with the same element at each index.
//    *
...
//    */
//  private[collection] def fillSparse[A](n: Int)(elem: A): FVector[A] = {
...
//    try System.getProperty("scala.collection.immutable.FVector.defaultApplyPreferredMaxLength",
//      "250").toInt
//    catch {
//      case _: SecurityException => 250
//    }
//
//  private val emptyIterator = new NewFVectorIterator(FVector0, 0, 0)
//}
//
//
...
//  */
//sealed abstract class FVector[+A] private[immutable] (private[immutable] final val prefix1: Arr1)
//  extends AbstractSeq[A]
//    with IndexedSeq[A]
//    with IndexedSeqOps[A, FVector, FVector[A]]
//    with StrictOptimizedSeqOps[A, FVector, FVector[A]]
//    with IterableFactoryDefaults[A, FVector]
...
//  override def updated[B >: A](index: Int, elem: B): FVector[B] = super.updated(index, elem)
//  override def appended[B >: A](elem: B): FVector[B] = super.appended(elem)
//  override def prepended[B >: A](elem: B): FVector[B] = super.prepended(elem)
//  override def prependedAll[B >: A](prefix: collection.IterableOnce[B]): FVector[B] = {
//    val k = prefix.knownSize
//    if(k == 0) this
//    else prependedAll0(prefix, k)
//  }
//
//  override final def appendedAll[B >: A](suffix: collection.IterableOnce[B]): FVector[B] = {
...
//      v
//    } else if (this.size < (k >>> Log2ConcatFaster) && prefix.isInstanceOf[FVector[_]]) {
//      var v  = prefix.asInstanceOf[FVector[B]]
...
//      }
//      v
//    } else if (this.size < (k >>> Log2ConcatFaster) && suffix.isInstanceOf[FVector[_]]) {
//      var v = suffix.asInstanceOf[FVector[B]]
//      val ri = this.reverseIterator
//      while (ri.hasNext) v = v.prepended(ri.next())
//      v
//    } else new FVectorBuilder[B].initFrom(this).addAll(suffix).result()
//  }
//
//  override def className = "FVector"
//
//  @inline override final def take(n: Int): FVector[A] = slice(0, n)
//  @inline override final def drop(n: Int): FVector[A] = slice(n, length)
//  @inline override final def takeRight(n: Int): FVector[A] = slice(length - mmax(n, 0), length)
//  @inline override final def dropRight(n: Int): FVector[A] = slice(0, length - mmax(n, 0))
//  override def tail: FVector[A] = slice(1, length)
//  override def init: FVector[A] = slice(0, length-1)
//
...
//
//  override def toVector: Vector[A] = Vector.from(this) // this
//
//  override protected def applyPreferredMaxLength: Int = FVector.defaultApplyPreferredMaxLength
//
//  override def stepper[S <: Stepper[_]](implicit shape: StepperShape[A, S]): S with EfficientSplit = {
//    val s = shape.shape match {
//      case StepperShape.IntShape    => new IntFVectorStepper(iterator.asInstanceOf[NewFVectorIterator[Int]])
//      case StepperShape.LongShape   => new LongFVectorStepper(iterator.asInstanceOf[NewFVectorIterator[Long]])
//      case StepperShape.DoubleShape => new DoubleFVectorStepper(iterator.asInstanceOf[NewFVectorIterator[Double]])
//      case _                        => shape.parUnbox(new AnyFVectorStepper[A](iterator.asInstanceOf[NewFVectorIterator[A]]))
//    }
...
//    if(this.isInstanceOf[BigFVector[_]]) {
//      val suffix = this.asInstanceOf[BigFVector[_]].suffix1
...
//  }
//
//  // The following definitions are needed for binary compatibility with ParFVector
//  private[collection] def startIndex: Int = 0
//  private[collection] def endIndex: Int = length
//  private[collection] def initIterator[B >: A](s: FVectorIterator[B]): Unit =
//    s.it = iterator.asInstanceOf[NewFVectorIterator[B]]
//}
//
//
...
//
//  override def tail: FVector[Nothing] = throw new UnsupportedOperationException("empty.tail")
//
//  override def init: FVector[Nothing] = throw new UnsupportedOperationException("empty.init")
//
...
//    else o match {
//      case that: FVector[_] => false
//      case o => super.equals(o)
//    }
//  }
//
//  override protected[this]def prependedAll0[B >: Nothing](prefix: collection.IterableOnce[B], k: Int): FVector[B] =
//    FVector.from(prefix)
//
//  override protected[this]def appendedAll0[B >: Nothing](suffix: collection.IterableOnce[B], k: Int): FVector[B] =
//    FVector.from(suffix)
//
...
//    val len1 = prefix1.length
//    if(len1 < WIDTH) new FVector1(copyPrepend1(elem, prefix1))
//    else new FVector2(wrap1(elem), 1, empty2, prefix1, len1+1)
//  }
//
//  override def map[B](f: A => B): FVector[B] = new FVector1(mapElems1(prefix1, f))
//
//  protected[this] def slice0(lo: Int, hi: Int): FVector[A] =
//    new FVector1(copyOfRange(prefix1, lo, hi))
//
//  override def tail: FVector[A] =
//    if(prefix1.length == 1) FVector0
//    else new FVector1(copyTail(prefix1))
//
//  override def init: FVector[A] =
//    if(prefix1.length == 1) FVector0
...
//    if(data1b ne null) new FVector1(data1b)
//    else super.prependedAll0(prefix, k)
//  }
//
//  override protected[this] def appendedAll0[B >: A](suffix: collection.IterableOnce[B], k: Int): FVector[B] = {
...
//                                 length0: Int = length0) =
//    new FVector2(prefix1, len1, data2, suffix1, length0)
//
//  @inline def apply(index: Int): A = {
...
//    else new FVector3(prefix1, len1, data2, WIDTH*(WIDTH-2) + len1, empty3, wrap2(suffix1), wrap1(elem), length0+1)
//  }
//
//  override def prepended[B >: A](elem: B): FVector[B] = {
...
//    else new FVector3(wrap1(elem), 1, wrap2(prefix1), len1+1, empty3, data2, suffix1, length0+1)
//  }
//
//  override def map[B](f: A => B): FVector[B] =
//    copy(prefix1 = mapElems1(prefix1, f), data2 = mapElems(2, data2, f), suffix1 = mapElems1(suffix1, f))
//
//  protected[this] def slice0(lo: Int, hi: Int): FVector[A] = {
//    val b = new FVectorSliceBuilder(lo, hi)
...
//private final class FVector3[+A](_prefix1: Arr1, private[immutable] val len1: Int,
...
//                                 length0: Int = length0) =
//    new FVector3(prefix1, len1, prefix2, len12, data3, suffix2, suffix1, length0)
//
//  @inline def apply(index: Int): A = {
//    if(index >= 0 && index < length0) {
//      val io = index - len12
...
//    else new FVector4(prefix1, len1, prefix2, len12, data3, (WIDTH-2)*WIDTH2 + len12, empty4, wrap3(copyAppend(suffix2, suffix1)), empty2, wrap1(elem), length0+1)
//  }
//
//  override def prepended[B >: A](elem: B): FVector[B] = {
...
//    else new FVector4(wrap1(elem), 1, empty2, 1, wrap3(copyPrepend(prefix1, prefix2)), len12+1, empty4, data3, suffix2, suffix1, length0+1)
//  }
//
//  override def map[B](f: A => B): FVector[B] =
...
//private final class FVector4[+A](_prefix1: Arr1, private[immutable] val len1: Int,
//                                 private[immutable] val prefix2: Arr2, private[immutable] val len12: Int,
...
//                                 length0: Int = length0) =
//    new FVector4(prefix1, len1, prefix2, len12, prefix3, len123, data4, suffix3, suffix2, suffix1, length0)
//
//  @inline def apply(index: Int): A = {
//    if(index >= 0 && index < length0) {
//      val io = index - len123
...
//    else new FVector5(prefix1, len1, prefix2, len12, prefix3, len123, data4, (WIDTH-2)*WIDTH3 + len123, empty5, wrap4(copyAppend(suffix3, copyAppend(suffix2, suffix1))), empty3, empty2, wrap1(elem), length0+1)
//  }
//
//  override def prepended[B >: A](elem: B): FVector[B] = {
...
//    else new FVector5(wrap1(elem), 1, empty2, 1, empty3, 1, wrap4(copyPrepend(copyPrepend(prefix1, prefix2), prefix3)), len123+1, empty5, data4, suffix3, suffix2, suffix1, length0+1)
//  }
//
//  override def map[B](f: A => B): FVector[B] =
...
//private final class FVector5[+A](_prefix1: Arr1, private[immutable] val len1: Int,
//                                 private[immutable] val prefix2: Arr2, private[immutable] val len12: Int,
//                                 private[immutable] val prefix3: Arr3, private[immutable] val len123: Int,
...
//                                 length0: Int = length0) =
//    new FVector5(prefix1, len1, prefix2, len12, prefix3, len123, prefix4, len1234, data5, suffix4, suffix3, suffix2, suffix1, length0)
//
//  @inline def apply(index: Int): A = {
//    if(index >= 0 && index < length0) {
...
//    else new FVector6(prefix1, len1, prefix2, len12, prefix3, len123, prefix4, len1234, data5, (WIDTH-2)*WIDTH4 + len1234, empty6, wrap5(copyAppend(suffix4, copyAppend(suffix3, copyAppend(suffix2, suffix1)))), empty4, empty3, empty2, wrap1(elem), length0+1)
//  }
//
//  override def prepended[B >: A](elem: B): FVector[B] = {
...
//    else new FVector6(wrap1(elem), 1, empty2, 1, empty3, 1, empty4, 1, wrap5(copyPrepend(copyPrepend(copyPrepend(prefix1, prefix2), prefix3), prefix4)), len1234+1, empty6, data5, suffix4, suffix3, suffix2, suffix1, length0+1)
//  }
//
//  override def map[B](f: A => B): FVector[B] =
...
//private final class FVector6[+A](_prefix1: Arr1, private[immutable] val len1: Int,
//                                 private[immutable] val prefix2: Arr2, private[immutable] val len12: Int,
//                                 private[immutable] val prefix3: Arr3, private[immutable] val len123: Int,
//                                 private[immutable] val prefix4: Arr4, private[immutable] val len1234: Int,
...
//          new FVector4[A](prefix1, len1, prefix2, len12, prefix3, len123, data4, suffix3, suffix2, suffix1, len)
//        case 5 =>
//          val prefix2 = prefixOr(2, empty2)
//          val prefix3 = prefixOr(3, empty3)
...
//          new FVector5[A](prefix1, len1, prefix2, len12, prefix3, len123, prefix4, len1234, data5, suffix4, suffix3, suffix2, suffix1, len)
//        case 6 =>
//          val prefix2 = prefixOr(2, empty2)
//          val prefix3 = prefixOr(3, empty3)
//          val prefix4 = prefixOr(4, empty4)
...
//          new FVector6[A](prefix1, len1, prefix2, len12, prefix3, len123, prefix4, len1234, prefix5, len12345, data6, suffix5, suffix4, suffix3, suffix2, suffix1, len)
//      }
//      res
//    }
//  }
...
//    s"FVectorSliceBuilder(lo=$lo, hi=$hi, len=$len, pos=$pos, maxDim=$maxDim)"
//
//  private[immutable] def getSlices: Array[Array[AnyRef]] = slices
//}
//
//
//final class FVectorBuilder[A] extends ReusableBuilder[A, FVector[A]] {
//
...
//      case 3 =>
//        val v2 = v.asInstanceOf[FVector2[_]]
//        val d2 = v2.data2
...
//        a2(d2.length+1) = a1
//      case 5 =>
//        val v3 = v.asInstanceOf[FVector3[_]]
//        val d3 = v3.data3
//        val s2 = v3.suffix2
...
//        a2(s2.length) = a1
//      case 7 =>
//        val v4 = v.asInstanceOf[FVector4[_]]
//        val d4 = v4.data4
//        val s3 = v4.suffix3
//        val s2 = v4.suffix2
...
//        a2(s2.length) = a1
//      case 9 =>
//        val v5 = v.asInstanceOf[FVector5[_]]
//        val d5 = v5.data5
//        val s4 = v5.suffix4
//        val s3 = v5.suffix3
//        val s2 = v5.suffix2
...
//        a2(s2.length) = a1
//      case 11 =>
//        val v6 = v.asInstanceOf[FVector6[_]]
//        val d6 = v6.data6
//        val s5 = v6.suffix5
//        val s4 = v6.suffix4
//        val s3 = v6.suffix3
...
//      else addFVector(v.asInstanceOf[FVector[A]])
//    case _ =>
//      super.addAll(xs)
//  }
//
...
//  }
//
//  def result(): FVector[A] = {
//    val len = len1 + lenRest
//    val realLen = len - offset
//    if(realLen == 0) FVector.empty
//    else if(len <= WIDTH) {
//      if(realLen == WIDTH) new FVector1(a1)
//      else new FVector1(copyOf(a1, realLen))
//    } else if(len <= WIDTH2) {
//      val i1 = (len-1) & MASK
//      val i2 = (len-1) >>> BITS
...
//      new FVector2(prefix1, WIDTH-offset, data, suffix1, realLen)
//    } else if(len <= WIDTH3) {
//      val i1 = (len-1) & MASK
...
//      val len1 = prefix1.length
//      val len12 = len1 + prefix2.length*WIDTH
//      new FVector3(prefix1, len1, prefix2, len12, data, suffix2, suffix1, realLen)
//    } else if(len <= WIDTH4) {
//      val i1 = (len-1) & MASK
//      val i2 = ((len-1) >>> BITS) & MASK
...
//      val len12 = len1 + prefix2.length*WIDTH
//      val len123 = len12 + prefix3.length*WIDTH2
//      new FVector4(prefix1, len1, prefix2, len12, prefix3, len123, data, suffix3, suffix2, suffix1, realLen)
//    } else if(len <= WIDTH5) {
//      val i1 = (len-1) & MASK
//      val i2 = ((len-1) >>> BITS) & MASK
//      val i3 = ((len-1) >>> BITS2) & MASK
...
//      val len123 = len12 + prefix3.length*WIDTH2
//      val len1234 = len123 + prefix4.length*WIDTH3
//      new FVector5(prefix1, len1, prefix2, len12, prefix3, len123, prefix4, len1234, data, suffix4, suffix3, suffix2, suffix1, realLen)
//    } else {
//      val i1 = (len-1) & MASK
//      val i2 = ((len-1) >>> BITS) & MASK
//      val i3 = ((len-1) >>> BITS2) & MASK
...
//      val len1234 = len123 + prefix4.length*WIDTH3
//      val len12345 = len1234 + prefix5.length*WIDTH4
//      new FVector6(prefix1, len1, prefix2, len12, prefix3, len123, prefix4, len1234, prefix5, len12345, data, suffix5, suffix4, suffix3, suffix2, suffix1, realLen)
//    }
//  }
//
//  override def toString: String =
//    s"FVectorBuilder(len1=$len1, lenRest=$lenRest, offset=$offset, depth=$depth)"
//
...
//
//
///** Compile-time definitions for FVector. No references to this object should appear in bytecode. */
//private[immutable] object FVectorInline {
...
//
//
///** Helper methods and constants for FVector. */
//private object FVectorStatics {
//
...
//
//
//private final class NewFVectorIterator[A](v: FVector[A], private[this] var totalLength: Int, private[this] val sliceCount: Int) extends Iterator[A] with java.lang.Cloneable {
//
...
//
//  //override def toString: String =
//  //  s"NewFVectorIterator(v=$v, totalLength=$totalLength, sliceCount=$sliceCount): a1len=$a1len, len1=$len1, i1=$i1, sliceEnd=$sliceEnd"
//
...
//    null // v.slice(i1-len1+totalLength, totalLength)
//  def toFVector: FVector[A] =
//    v.slice(i1-len1+totalLength, totalLength)
//
//  protected[immutable] def split(at: Int): NewFVectorIterator[A] = {
//    val it2 = clone().asInstanceOf[NewFVectorIterator[A]]
//    it2.take(at)
//    drop(at)
//    it2
//  }
...
//
//
//private abstract class FVectorStepperBase[A, Sub >: Null <: Stepper[A], Semi <: Sub](it: NewFVectorIterator[A])
//  extends Stepper[A] with EfficientSplit {
//
//  protected[this] def build(it: NewFVectorIterator[A]): Semi
//
...
//}
//
//private class AnyFVectorStepper[A](it: NewFVectorIterator[A])
//  extends FVectorStepperBase[A, AnyStepper[A], AnyFVectorStepper[A]](it) with AnyStepper[A] {
//  protected[this] def build(it: NewFVectorIterator[A]) = new AnyFVectorStepper(it)
//  def nextStep(): A = it.next()
//}
//
//private class DoubleFVectorStepper(it: NewFVectorIterator[Double])
//  extends FVectorStepperBase[Double, DoubleStepper, DoubleFVectorStepper](it) with DoubleStepper {
//  protected[this] def build(it: NewFVectorIterator[Double]) = new DoubleFVectorStepper(it)
//  def nextStep(): Double = it.next()
//}
//
//private class IntFVectorStepper(it: NewFVectorIterator[Int])
//  extends FVectorStepperBase[Int, IntStepper, IntFVectorStepper](it) with IntStepper {
//  protected[this] def build(it: NewFVectorIterator[Int]) = new IntFVectorStepper(it)
//  def nextStep(): Int = it.next()
//}
//
//private class LongFVectorStepper(it: NewFVectorIterator[Long])
//  extends FVectorStepperBase[Long, LongStepper, LongFVectorStepper](it) with LongStepper {
//  protected[this] def build(it: NewFVectorIterator[Long]) = new LongFVectorStepper(it)
//  def nextStep(): Long = it.next()
//}
//
//
//// The following definitions are needed for binary compatibility with ParFVector
//private[collection] class FVectorIterator[+A](_startIndex: Int, private[this] var endIndex: Int) extends AbstractIterator[A] {
//  private[immutable] var it: NewFVectorIterator[A @uncheckedVariance] = _
//  def hasNext: Boolean = it.hasNext
//  def next(): A = it.next()
//  private[collection] def remainingElementCount: Int = it.size
//  private[collection] def remainingFVector: FVector[A] = it.toFVector
//}
